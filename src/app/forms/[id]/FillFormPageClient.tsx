...
export type FillFormPageClientProps = {
  templateId: string;
  ...
  React.useEffect(() => {
    ...
    async function load() {
      ...
      if (submissionId) {
        ...
        const templateForSubmission = hydrationTemplateForSubmission(sub, loadedTemplate);
        ...
        const inSameScope = (row: SubmissionRecord) => {
          ...
          return row.folderId === undefined || row.folderId === "";
        };
        const scopedSorted = sorted.filter(inSameScope);
        const mostRecentId = scopedSorted[0]?.id;

        const prior = scopedSorted.find((s) => s.id !== sub.id) ?? null;
        ...
        if (blockedHistoricalFinal && !cancelled) {
          setStatus(
            "This is an older finalized entry for this form — showing read-only. Only your newest submission for this folder and form template stays editable.",
          );
        }
      } else {
      ...
      <main className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-6 py-8">
        ...
        {!template ? (
          ...
          <form className="flex flex-col gap-6">
            {openerButtons.length > 0 && !blockedEdit ? (
              ...
              <div className="flex flex-col gap-4">
                ...
                {openRounds.map((inst) => {
                  ...
                  return (
                    <div
                      key={inst.id}
                      className={`relative ui-card flex flex-col gap-4 border-amber-200/80 pt-1 ring-1 ring-amber-900/5 ${blockedEdit ? "pr-4" : "pr-11"}`}
                    >
                      {!blockedEdit ? (
                        <button
                          type="button"
            ...
            <fieldset disabled={blockedEdit} className="flex min-w-0 flex-col gap-6 border-0 p-0">
