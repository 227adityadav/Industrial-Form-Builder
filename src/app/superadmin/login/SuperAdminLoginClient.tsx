...
export function SuperAdminLoginClient() {
  ...
  const next = safeInternalPath(params.get("next"), "/superadmin/builder");
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setError(null);
    logAuthClient("login_submit", { role: "superadmin", next });
    const res = await fetch("/api/auth/login", {
      method: "POST",
      headers: { "content-type": "application/json" },
      credentials: "include",
      body: JSON.stringify({ role: "superadmin", password }),
    });
    setLoading(false);
  ...
  return (
    <AuthShell accent="indigo" eyebrow="Super administrator" title="Superadmin sign in">
