...
export function SpcLoginClient() {
  const params = useSearchParams();
  const next = safeInternalPath(params.get("next"), "/spc");

  const [username, setUsername] = React.useState("SPC");
  const [password, setPassword] = React.useState("spc123");
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setError(null);
    logAuthClient("login_submit", { role: "spc", next });
    const res = await fetch("/api/auth/login", {
      method: "POST",
      headers: { "content-type": "application/json" },
      credentials: "include",
      body: JSON.stringify({ role: "spc", username, password }),
    });
    setLoading(false);
  ...
  return (
    ...
    >
      <form className="flex flex-col gap-4" onSubmit={onSubmit}>
        ...
        <Link
          className="ui-btn-secondary w-full justify-center no-underline hover:no-underline"
          href="/spc/msa"
        >
          MSA
        </Link>
