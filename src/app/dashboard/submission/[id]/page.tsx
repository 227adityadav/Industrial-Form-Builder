...
function formatTopValue(field: TopField, raw: unknown): string {
  ...
  if (field.inputType === "signature" && isDigitalSignatureValue(raw)) {
    return `Digital signature · ${new Date(raw.signedAt).toLocaleString()}`;
  }
  if (field.inputType === "file" && isUploadedFileFieldValue(raw)) {
    return `${raw.fileName} · ${new Date(raw.uploadedAt).toLocaleString()}`;
  }
    ...
    <div className="app-page">
      ...
      >
        ...
        <a
          ...
          href={submission.folderId ? `/dashboard/folder/${submission.folderId}` : "/dashboard"}
        >
          ← Back
        </a>
      </PageHeader>
